 ...
 *************************************************************************/
struct conscell * bufillarray(struct conscell *form)
{      struct conscell **e,*n,*l;
       ...
       {   if (ExtractArray(form->carp,&array))
           {  form = form->cdrp;
              ...
              n = form->carp;
              l = NULL;
              size = FIX(ARRAY(array)->info->carp)->atom;
              for(i=0L;i<size;i++)
              {    e = GetArrayIndex(ARRAY(array)->base,i,size);
                   if (n != NULL)
  ...
  ERR: ierror("fillarray");  /*  doesn't return  */
